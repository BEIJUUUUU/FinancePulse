...
#### 数据持久化
推送历史库 (`history.db`)、运行配置 (`settings.json`) 与表格归档 (`财经热点汇总.csv`) 自动写入容器内 `/app/data`，配合 compose 中的 `./data:/app/data` 挂载，容器重建升级后数据不丢失。

#### WebUI 可视化控制台 (端口 6888)
容器启动后自动开启 Web 控制台，浏览器访问 `http://NAS_IP:6888` 即可使用：
- **可视化配置编辑**：邮箱/授权码/大模型 Key/思考强度/抓取条数/信源/关注领域/定时时段/自选与突发关键词全部表单化编辑，一键保存自动持久化；
- **手动运行**：点击「立即运行一次」即刻触发完整抓取与推送流程；
- **资讯预览**：表格化展示最新一次运行的资讯、利好/利空分级与 AI 视点；
- **推送历史与实时日志**：最近 30 条推送记录与运行日志每 3 秒自动刷新。
- 可选安全令牌：compose 中设置 `WEBUI_PASSWORD` 环境变量后，打开控制台需输入访问令牌。仅建议在可信局域网内使用。
- Windows 桌面用户亦可独立运行 `python webui.py` 启用控制台 (默认端口 6888，可用环境变量 `WEBUI_PORT` 修改)。
